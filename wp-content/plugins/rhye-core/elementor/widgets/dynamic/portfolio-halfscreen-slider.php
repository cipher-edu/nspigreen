...
class Rhye_Widget_Portfolio_Halfscreen_Slider extends Rhye_Widget_Base {
	...
	public function wpml_widgets_to_translate_filter( $widgets ) {
		...
		$widgets[ $name ] = array(
			...
			'integration-class' => 'WPML_Elementor_Rhye_Widget_Portfolio_Halfscreen_Slider',
		);
	...
	protected function render() {
		...
		$this->add_render_attribute(
			'overlay',
			array(
				'class' => array( 'slider__overlay', 'overlay', 'z-50', 'd-lg-none' ),
			)
		);
		...
		$this->add_render_attribute(
			'slider_images',
			array(
				'class'                  => array( 'slider-fullscreen-projects__images', 'slider-halfscreen-projects__images', 'swiper', 'swiper-container', 'js-slider-fullscreen-projects__images' ),
		...
		$this->add_render_attribute(
			'slider_content',
			array(
				'class' => array( 'slider-halfscreen-projects__content', 'container-fluid', 'container-fluid_paddings', 'swiper', 'swiper-container', 'js-slider-fullscreen-projects__content' ),
			)
		);
		...
		$this->add_render_attribute(
			'container',
			array(
				'class' => array( $settings['container'], 'h-100', 'z-50', 'position-relative', 'slider-halfscreen-projects__container' ),
			)
		);
		...
		$this->add_render_attribute(
			'col_content',
			array(
				'class' => array( 'col-lg-6', 'align-self-lg-center', 'z-100', 'slider-halfscreen-projects__col', 'slider-halfscreen-projects__col-content', 'pointer-events-none', $settings['content_position'] ),
			)
		);
		...
		$this->add_render_attribute(
			'col_images',
			array(
				'class' => array( 'col-lg-6', 'slider-halfscreen-projects__col' ),
			)
		);
		...
		switch ( $settings['container'] ) {
			...
			case 'container-fluid container-fluid_paddings': {
				$this->add_render_attribute(
					'slider_wrapper_arrows',
					array(
						'class' => array( 'slider__wrapper-arrows_right-mini', 'slider__wrapper-arrows_bottom' ),
					)
				);
		...
		if ( $settings['container'] === 'container-fluid container-fluid_paddings' ) {
			if ( $settings['content_position'] === 'order-lg-2' ) {
				$this->add_render_attribute(
					'slider_images',
					array(
						'class' => array( 'slider-halfscreen-projects__images_reduced-width-left' ),
					)
				);
				...
				$this->add_render_attribute(
					'slider_images',
					array(
						'class' => array( 'slider-halfscreen-projects__images_reduced-width-right' ),
					)
				);
		...
		<?php if ( ! empty( $posts ) ) : ?>
			...
			if ( $first_path && $first_path[1] && $first_path[2] ) {
